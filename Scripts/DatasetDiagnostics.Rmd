...
# load in the data
data <- fread(file.path("C:/Users/oaw001/OneDrive - University of the Sunshine Coast/DataReformatting", species, paste0(species, "_formatted.csv")))

data <- data %>% dplyr::filter(!GeneralisedActivity== "") # removing the NA data
...
data <- data %>%
  ...
  mutate(
    change_point = as.integer(!is.na(lag(Activity)) & lag(Activity) != Activity),
...
seq_trans <- data %>%
  group_by(ID, sequence) %>%
  dplyr::summarise(transitons = max(event))
...
```{r trace examples, echo = FALSE, fig.width = 20, fig.height = 10}
plotTraceExamples <- function(behaviours, data, individuals, n_samples) {
  
  data <- data %>% dplyr::filter(ID %in% sample(unique(data$ID), individuals))
...
plot_behaviour <- function(behaviour, n_samples, data) {
  df <- data %>%
    ...
    group_by(ID, Activity) %>%
    slice(1:n_samples) %>%
    dplyr::mutate(relative_time = row_number())
...
tracePlots <- plotTraceExamples(
  ...
  n_samples = 3 * sample_rate # 2 seconds
)
